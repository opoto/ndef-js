...
var ndef = {
     ...
     * @type byte array or String, containing zero to 255 bytes, optional
     * @id byte array or String, containing zero to 255 bytes, optional
     * @payload byte array or String, containing zero to (2 ** 32 - 1) bytes, optional
     *
...
var stringifier = {
    ...
    recordStr: function (rectype, type, id, payload, separator) {
        if(!separator) { separator = "\n"; }
        rstr = "";
        rstr += rectype;
        if (!util.empty(type)) {
    ...
    printRecord: function (record, separator) {
        ...
        var result = "";
        var tnfStr = tnfToString(record.tnf);
        ...
        switch(record.tnf) {
            case ndef.TNF_WELL_KNOWN:
                ...
                break;
            case ndef.TNF_EMPTY: // should have empty type, id and payload
            case ndef.TNF_MIME_MEDIA:
            case ndef.TNF_ABSOLUTE_URI:
            case ndef.TNF_EXTERNAL_TYPE: // AAR contains strings, other types could contain binary data
                result += this.recordStr(tnfStr, record.type, record.id, record.payload, separator);
                break;
            default:
    ...
    printWellKnown: function (record, separator) {
        ...
        switch(record.type) {
            case ndef.RTD_TEXT:
                result += this.recordStr("Text Record", record.type, record.id, 
                            ndef.text.decodePayload(record.payload), separator);
                break;
            case ndef.RTD_URI:
                result += this.recordStr("URI Record", record.type, record.id, 
                            ndef.uri.decodePayload(record.payload), separator);
                break;
            case ndef.RTD_SMART_POSTER:
                // the payload of a smartposter is a NDEF message
                result += this.recordStr("Smart Poster", record.type, record.id, separator + "    "  +
                            stringifier.printRecords(ndef.decodeMessage(record.payload), separator + "    "), separator);
                break;
            default:
                // attempt to display other types
                result += this.recordStr(record.type + " Record", record.type, record.id, 
                            record.payload, separator);
        }
