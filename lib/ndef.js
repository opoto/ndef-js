...
var stringifier = {
    ...
    printRecord: function (record, separator) {
        ...
        switch(record.tnf) {
            case ndef.TNF_EMPTY:
                result += this.recordStr("Empty Record", undefined, undefined, undefined, separator);
                break;
            case ndef.TNF_WELL_KNOWN:
                result += stringifier.printWellKnown(record, separator);
                break;
            case ndef.TNF_MIME_MEDIA:
                result += this.recordStr("MIME Media", record.type, record.id, record.payload, separator);
                break;
            case ndef.TNF_ABSOLUTE_URI:
                result += this.recordStr("Absolute URI", record.type, record.id, record.payload, separator);
                break;;
            case ndef.TNF_EXTERNAL_TYPE:
                // AAR contains strings, other types could
                // contain binary data
                result += this.recordStr("External", record.type, record.id, record.payload, separator);
                break;
            default:
